...
object Display {
  ...
  def Synchronizing(sync: Boolean) = synchronized { synchronizing = sync }

  /* Await a signal for the end of frame drawing from the GUI thread, using the given semaphore */
  private def frameWait(sync: Semaphore) = synchronized {
...
class Display(boids: Set[Boid]) extends JFrame {
  import Display._
  // Constants
  ...
  def draw() = { board.paintImmediately(0, 0, bwidth, bheight); 
                 frameWait(sync)
               }
